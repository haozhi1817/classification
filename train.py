...
from data import build_dataloader, build_dataloader_v2, build_dataloader_mixup
from model import build_backbone, build_loss
from utils.utils import read_csv, metric, get_label_dict
...
log_path = r"D:\workspace\convnext\log_resnet_coswarm"
ckpt_path = r"D:\workspace\convnext\ckpt_resnet_coswarm"
...
def main():
    ...
    # dataset
    if aug_mod == "randaug":
        train_loader = build_dataloader_v2(
            train_folder, batch_size=batch_size, img_size=img_size, mode="train"
        )
    else:
    ...
    if mix_up:
        train_loader = build_dataloader_mixup(
            train_folder,
    ...
    print('lr_scheduler: ', scheduler_mode)
    ...
    for epoch in range(init_epoch, num_epoch):
        ...
        for idx, (imgs, labels, pathes) in enumerate(train_loader):
            ...
            writer.add_scalar("ce_loss", loss, global_step=epoch * num_batch + idx)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            print(
